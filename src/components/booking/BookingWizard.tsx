...
export function BookingWizard({ initialPackage }: BookingWizardProps) {
  ...
  // Step 4 -> 5: Atomic Booking Creation with duplicate submission prevention
  const handleProceedToPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isSubmitting || isRedirecting) return; // Prevent duplicate concurrent submissions
  ...
  // Step 5: PayMongo Gateway Redirect Handler with duplicate submission prevention
  const handlePayMongoRedirect = async () => {
    if (isRedirecting || isSubmitting) return; // Prevent duplicate redirects
    if (!createdBookingId) {
