...
DECLARE
    ...
    v_assigned_unit_id UUID;
    v_assigned_unit_status TEXT;
...
BEGIN
    ...
    SELECT status, public_id, balance_amount, assigned_unit_id
    INTO v_current_status, v_booking_public_id, v_balance_amount, v_assigned_unit_id
    ...
    IF p_target_status = 'COMPLETED' THEN
        IF COALESCE(v_balance_amount, 0) > 0 THEN
            RAISE EXCEPTION 'Completion blocked: Booking % has an outstanding balance of PHP %',
                v_booking_public_id, TRIM(TO_CHAR(v_balance_amount, '999,999,990.00'));
        END IF;

        IF v_assigned_unit_id IS NOT NULL THEN
            SELECT status
            INTO v_assigned_unit_status
            FROM public.inventory_units
            WHERE id = v_assigned_unit_id
              AND tenant_id = p_tenant_id
              AND is_deleted = FALSE
            FOR UPDATE;

            IF NOT FOUND THEN
                RAISE EXCEPTION 'Completion blocked: Assigned inventory unit was not found';
            END IF;

            IF v_assigned_unit_status = 'IN_USE' THEN
                RAISE EXCEPTION 'Completion blocked: Assigned inventory unit must be inspected and returned to READY_TO_DEPLOY or moved to UNDER_REPAIR';
            END IF;
        END IF;
    END IF;
