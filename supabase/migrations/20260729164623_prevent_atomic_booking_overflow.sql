-- Serialize booking capacity decisions and remove the obsolete booking RPC.

ALTER TABLE public.inventory_locks
    ADD COLUMN IF NOT EXISTS event_date DATE;

UPDATE public.inventory_locks AS inventory_lock
SET event_date = booking.event_date
FROM public.bookings AS booking
WHERE inventory_lock.session_id = booking.id::TEXT
  AND inventory_lock.event_date IS NULL;
...
DROP INDEX IF EXISTS public.idx_inventory_locks_active;
CREATE INDEX idx_inventory_locks_active
    ON public.inventory_locks (tenant_id, package_id, event_date, expires_at);
...
BEGIN
    ...
    -- One capacity decision at a time for this tenant/package/date tuple.
...
) IS 'Creates a booking and date-scoped inventory lock after serialized, transaction-time capacity validation.';

DROP FUNCTION IF EXISTS public.create_booking_atomic(
    UUID, UUID, UUID, DATE, TIME WITHOUT TIME ZONE, INTEGER,
