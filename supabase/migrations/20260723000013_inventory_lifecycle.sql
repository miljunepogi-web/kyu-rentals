-- ============================================================================
-- KYU RENTALS — MIGRATION 00013: INVENTORY LIFECYCLE
-- Version: 1.0.0
-- Date: 2026-07-23
-- Purpose:
--   1. Create inventory_maintenance_logs table for full unit lifecycle auditing.
--   2. Create public.update_inventory_unit_status_admin() atomic RPC with:
--      - auth.uid() identity verification (forging prevention)
--      - RBAC role authorization (admin, super_admin, franchise_owner)
--      - Tenant isolation assertion
--      - FOR UPDATE row lock + optimistic concurrency check
--      - Legal state machine transition matrix
--      - ROW_COUNT assertions on UPDATE and INSERT
...
CREATE SEQUENCE IF NOT EXISTS public.inventory_maintenance_logs_public_id_seq START WITH 1 INCREMENT BY 1;
GRANT USAGE, SELECT ON SEQUENCE public.inventory_maintenance_logs_public_id_seq TO postgres, authenticated, service_role;

-- ----------------------------------------------------------------------------
-- 2. INVENTORY MAINTENANCE LOGS TABLE
-- ----------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS public.inventory_maintenance_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    public_id TEXT NOT NULL UNIQUE DEFAULT public.generate_public_id('MNT', 'inventory_maintenance_logs_public_id_seq'),
...
COMMENT ON TABLE public.inventory_maintenance_logs IS
  'Immutable audit log of every admin-initiated inventory unit status transition, '
  'including mandatory reason and optional free-text maintenance notes.';

CREATE INDEX IF NOT EXISTS idx_inventory_maintenance_logs_unit
    ON public.inventory_maintenance_logs (tenant_id, unit_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_inventory_maintenance_logs_performed_by
    ON public.inventory_maintenance_logs (performed_by);

-- ----------------------------------------------------------------------------
-- 3. ROW LEVEL SECURITY
-- ----------------------------------------------------------------------------

ALTER TABLE public.inventory_maintenance_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Tenant members view own maintenance logs"
...
CREATE POLICY "Service role full access maintenance logs"
    ON public.inventory_maintenance_logs
    FOR ALL
    TO service_role
    USING (TRUE)
...
-- ----------------------------------------------------------------------------
-- 4. ATOMIC INVENTORY UNIT STATUS TRANSITION RPC
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    v_is_valid_transition := CASE
        ...
        WHEN v_current_status = 'RETIRED'           THEN FALSE  -- terminal state
        ELSE FALSE
    END;
    ...
    -- ------------------------------------------------------------------------
    -- 6. ATOMIC MUTATION — UPDATE UNIT STATUS
    -- ------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------
    -- 7. ATOMIC MUTATION — INSERT MAINTENANCE LOG
    -- ------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.update_inventory_unit_status_admin IS
  'Hardened atomic inventory unit status transition RPC. Validates caller identity against '
  'auth.uid(), enforces RBAC, enforces tenant isolation, applies legal state machine matrix, '
