-- ============================================================================
-- KYU RENTALS - RELEASE INVENTORY LOCK AFTER PAYMENT
-- Date: 2026-07-28
-- Purpose:
--   A confirmed booking is counted by the availability engine, so its temporary
--   checkout lock must be removed in the same transaction that confirms payment.
-- ============================================================================
...
BEGIN
    ...
    v_new_balance := v_booking.balance_amount;
...
-- Repair confirmed bookings affected by the previous function definition.
DELETE FROM public.inventory_locks AS inventory_lock
USING public.bookings AS booking
WHERE inventory_lock.tenant_id = booking.tenant_id
  AND inventory_lock.session_id = booking.id::TEXT
